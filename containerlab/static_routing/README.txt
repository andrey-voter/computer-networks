...
1) generate_topo.py
Запуск: sudo python3 generate_topo.py
Описание: Данный скрипт создает файл test0.yml и на его основе запускает clab, после чего производит настройку всех узлов (прописывает адреса, маршруты и т. д.)
...
Описание: Предполагается, что скрипт запсукается после успешного запуска generate_topo.py
Данный скрипт содержит набор функций для подключения к запущенным контейнерам, запуска на них ряда команд и вывода форматированного рещультата.
В конце файла представлены примеры работы со скриптом, в которых проверяется ip-связность всех PC (и не только)

Ответ по пункту 1.2.3. При разрыве связи между R1 и R2, PC1 больше не может пинговать PC2, т. к. статический маршрут через R3 не прописан
